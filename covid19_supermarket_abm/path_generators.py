...
def sythetic_paths_generator(mu, sigma, entrance_nodes, till_nodes, exit_nodes, item_nodes,
    ...
    Third, it samples K random item nodes, chosen uniformly or using the weights parameter with replacement from
    item_nodes, which we denote by v_2, ... v_K+1.
...
def runtime_paths_generator(mu, sigma, entrance_nodes, till_nodes, exit_nodes, item_nodes
    ...
    The sequence v_1, ..., v_K+3 is a node sequence where the customer bought items, along the the entrance, till and exit
    nodes that they visited.
    During runtime the customers update their paths to the next node in the sequence according to given parameters.
...
def get_path_generator(path_generation: str = 'empirical', G: Optional[nx.Graph] = None,
                       ...
                       runtime_path_generator_args: Optional[list] = None):
    ...
    elif path_generation == 'runtime':
        assert G is not None
        assert runtime_path_generator_args is not None
        assert type(runtime_path_generator_args) is list
        assert len(runtime_path_generator_args) in [6, 7], \
            "If you use path_generation='runtime', " \
            "you need to input runtime_path_generator_args=" \
            "[mu, sigma, entrance_nodes, till_nodes, exit_nodes, item_nodes]"
        path_generator_function = runtime_paths_generator
        path_generator_args = runtime_path_generator_args
    else:
